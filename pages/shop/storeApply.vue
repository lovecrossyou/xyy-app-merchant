<template>
	<view class="apply_wrapper">
		...
		<view class="store_imgs">
			<view class="uploading_img_item" @click="upLoadLeader_identity_card_positive_side" style="border: none;">
			...
			<view class="uploading_img_item" @click="upLoadLeader_identity_card_negative_side" style="border: none;">
				...
				<image v-if="shopInfo.leader_identity_card.negative_side.length!=0" :src="shopInfo.leader_identity_card.negative_side" mode="" class="add_img"></image>
				<image v-else src="../../static/shop/addImg@2x.png" mode="" class="add_img"></image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			upLoadBrand_sale_protocol_image(){
				let that = this;
				uni.chooseImage({
					success: chooseImageRes => {
						const tempFilePaths = chooseImageRes.tempFilePaths;
						api.uploader(tempFilePaths[0], res => {
							that.shopInfo.brand_sale_protocol_image = res.image_path;
						})
					}
				});
			},
			...
			upLoadLeader_identity_card_positive_side(){
				let that = this;
				uni.chooseImage({
					success: chooseImageRes => {
						const tempFilePaths = chooseImageRes.tempFilePaths;
						api.uploader(tempFilePaths[0], res => {
							that.shopInfo.leader_identity_card.positive_side = res.image_path;
						})
					}
				});
			},
			upLoadLeader_identity_card_negative_side(){
				let that = this;
				uni.chooseImage({
					success: chooseImageRes => {
						const tempFilePaths = chooseImageRes.tempFilePaths;
						api.uploader(tempFilePaths[0], res => {
							that.shopInfo.leader_identity_card.negative_side = res.image_path;
						})
					}
				});
			},
